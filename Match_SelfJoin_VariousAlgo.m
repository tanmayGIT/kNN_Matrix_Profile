function wholetime = Match_SelfJoin_VariousAlgo(subFoldersTarget, subSeqLen, kNN_Uwant, whichAlgoToConsider, nCores)
...
tic
[keepAllTargetTogether, keepDataFileInfo] = ConcatenateAllSeries(subFoldersTarget); % concatenate all the time series
...
parfor divFilesInCluster = 1:1:n_work
    ...
    [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = ...
        ProcessTheFilesInParallel(keepAllTargetTogether, keepDataFileInfo, subSeqLen, kNN_Uwant, startFiles, endFiles, whichAlgoToConsider);
...
function [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = ...
    ProcessTheFilesInParallel(keepAllTargetTogether, keepDataFileInfoConcat, subSeqLen, kNN_Uwant, startMe, endMe, whichAlgoToConsider)

startFiles = keepDataFileInfoConcat{startMe}.DataStart;
endFiles = keepDataFileInfoConcat{endMe}.DataEnd;

if(whichAlgoToConsider == "sortBased")
    [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParSortBased(keepAllTargetTogether, subSeqLen, kNN_Uwant, startFiles, endFiles);
elseif (whichAlgoToConsider == "maxBased")
    [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParMaxBased(keepAllTargetTogether, subSeqLen, kNN_Uwant, startFiles, endFiles);
elseif (whichAlgoToConsider == "heapBased")
...
function [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParMaxBased(targetSeqComplete, subSeqLen, kNN_Uwant, startFiles, endFiles)
...
%             [maxDisvalForeachCol, maxRwidxForEachCol] = max(sorted_Disval(1:kNN_Uwant, :),[], 1); % columnwise sorting
            
            maxDisvalForeachCol = zeros(1,size(sorted_Disval,2));
            maxRwidxForEachCol = zeros(1,size(sorted_Disval,2));
            
            for iu = 1:1:size(sorted_Disval,2)
                [getMaxVal, getMaxIndx] = max(sorted_Disval(:, iu));
                maxDisvalForeachCol(1,iu) = getMaxVal;
                maxRwidxForEachCol(1,iu) = getMaxIndx;
            end
            ...
            sorted_Disval(1:kNN_Uwant,:) = sorted_Disval(1:kNN_Uwant,:);
            sorted_idx(1:kNN_Uwant,:) = sorted_idx(1:kNN_Uwant,:);
...
function [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParSortBased(targetSeqComplete, subSeqLen, kNN_Uwant, startFiles, endFiles)
...
%             [sorted_CrossingDisval, sorted_Crossingidx] = sort(sorted_Disval(:, :), 1); % columnwise sorting
            ...
            sorted_CrossingDisval = zeros(size(sorted_Disval));
            sorted_Crossingidx = zeros(size(sorted_Disval));
            
            for iu = 1:1:size(sorted_Disval,2)
                [getSortedVal, getSortedIndx] = max(sorted_Disval(:, iu));
                sorted_CrossingDisval(:,iu) = getSortedVal(:);
                sorted_Crossingidx(:,iu) = getSortedIndx(:);
            end
