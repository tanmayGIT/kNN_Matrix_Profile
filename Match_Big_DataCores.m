function [wholetime] = Match_Big_DataCores(subFoldersTarget, querySeqComplete, subSeqLen,...
    kNN_Uwant, whichDimToConsider,nCores)
...
if isempty(gcp('nocreate')) % if the current parallel pool is empty
    parpool(getAllCrores);
    fprintf('The parallel pool exist from before and the opened pool are : %d \n', getAllCrores);
elseif (pool.NumWorkers ~= getAllCrores)  % if the current parallel pool is not equal to available number of workers
    delete(gcp('nocreate'));
    parpool(getAllCrores);
