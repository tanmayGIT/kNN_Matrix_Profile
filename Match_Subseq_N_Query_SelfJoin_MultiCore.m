function wholetime = Match_Subseq_N_Query_SelfJoin_MultiCore(subFoldersTarget, subSeqLen, kNN_Uwant)
...
tic
[keepAllTargetTogether, keepDataFileInfo, validSubSeq] = ConcatenateAllSeries(subFoldersTarget, subSeqLen); % concatenate all the time series
...
parfor divFilesInCluster = 1:1:n_work
    ...
    [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery, keepAllFileInfoIndex,keepAllFileInfoStart, keepAllFileInfoEnd] = ...
        ProcessTheFilesInParallel(keepAllTargetTogether, keepDataFileInfo, validSubSeq, subSeqLen, kNN_Uwant, startFiles, endFiles);
    ...
    keep_KNN_Cell{divFilesInCluster}.FileInfoIndex = keepAllFileInfoIndex;
    keep_KNN_Cell{divFilesInCluster}.FileInfoStart = keepAllFileInfoStart;
    keep_KNN_Cell{divFilesInCluster}.FileInfoEnd = keepAllFileInfoEnd;
end
...
keep_KNN_DataFileInfoIndexMerge = zeros((kNN_Uwant * n_work),numOfpossibleQuery);
keep_KNN_DataFileInfoStartMerge = zeros((kNN_Uwant * n_work),numOfpossibleQuery);
keep_KNN_DataFileInfoEndMerge = zeros((kNN_Uwant * n_work),numOfpossibleQuery);
...
for mergeMe = 1:1:n_work
    ...
    keep_KNN_DataFileInfoIndexMerge(indexMe:((indexMe+kNN_Uwant)-1),:) = keep_KNN_Cell{mergeMe}.FileInfoIndex(:,:);
    keep_KNN_DataFileInfoStartMerge(indexMe:((indexMe+kNN_Uwant)-1),:) = keep_KNN_Cell{mergeMe}.FileInfoStart(:,:);
    keep_KNN_DataFileInfoEndMerge(indexMe:((indexMe+kNN_Uwant)-1),:) = keep_KNN_Cell{mergeMe}.FileInfoEnd(:,:);
...
keepAllNearestNNInfoINN = zeros(kNN_Uwant,numOfpossibleQuery);
keepAllNearestNNInfoIndex = zeros(kNN_Uwant,numOfpossibleQuery);
keepAllNearestNNInfoFileStart = zeros(kNN_Uwant,numOfpossibleQuery);
keepAllNearestNNInfoFileEnd = zeros(kNN_Uwant,numOfpossibleQuery);
...
for goEachQuery = 1:1:numOfpossibleQuery
    ...
    for iNN = 1:1:kNN_Uwant
        ...
        if(isfinite(sorted_Disval(iNN,goEachQuery))) % if the distance at that index is finite
            ...
            seriesStart = keep_KNN_DataFileInfoStartMerge(getSortedIdx,goEachQuery);
            seriesEnd = keep_KNN_DataFileInfoEndMerge(getSortedIdx,goEachQuery);
            ...
            if((seriesStart <= pickIndex) && (seriesEnd >= pickIndex) )
                ...
                keepAllNearestNNInfoINN(iNN, goEachQuery)= iNN;
                keepAllNearestNNInfoIndex(iNN, goEachQuery) = keep_KNN_DataFileInfoIndexMerge(getSortedIdx,goEachQuery);

                keepAllNearestNNInfoFileStart(iNN, goEachQuery)= forward;
                keepAllNearestNNInfoFileEnd(iNN, goEachQuery)= ((forward)+subSeqLen)-1;

                keepFileIndxEntryOnly(iNN, goEachQuery) = keep_KNN_DataFileInfoIndexMerge(getSortedIdx,goEachQuery);
...
function [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery, keepAllFileInfoIndex,keepAllFileInfoStart, keepAllFileInfoEnd] = ...
                ProcessTheFilesInParallel(keepAllTargetTogether, keepDataFileInfoConcat, validSubSeq, subSeqLen, kNN_Uwant, startMe, endMe)

 startFiles = keepDataFileInfoConcat{startMe}.DataStart;
 endFiles = keepDataFileInfoConcat{endMe}.DataEnd;

[keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParMaxHeapBased(keepAllTargetTogether, validSubSeq, subSeqLen, kNN_Uwant, startFiles, endFiles);

% Now verify each index and put in the right cell order
keepAllFileInfoIndex = zeros(size(keep_KNN_Index_ToQuery,1), size(keep_KNN_Index_ToQuery,2));
keepAllFileInfoStart = zeros(size(keep_KNN_Index_ToQuery,1), size(keep_KNN_Index_ToQuery,2));
keepAllFileInfoEnd = zeros(size(keep_KNN_Index_ToQuery,1), size(keep_KNN_Index_ToQuery,2));

for io = 1:1:size(keep_KNN_Index_ToQuery,1)
    for jo = 1:1:size(keep_KNN_Index_ToQuery,2)
        getIndex = keep_KNN_Index_ToQuery(io, jo);
        enterFlag = false;
        for pickFiles = 1:1:size(keepDataFileInfoConcat,1)
            getFullFileInfo = keepDataFileInfoConcat{pickFiles,1};

            if( (getFullFileInfo.DataStart <= getIndex) && ...
                    (getIndex <= getFullFileInfo.DataEnd) )
                keepAllFileInfoIndex(io, jo) = getFullFileInfo.FileNum;
                keepAllFileInfoStart(io, jo) = getFullFileInfo.DataStart;
                keepAllFileInfoEnd(io, jo) = getFullFileInfo.DataEnd;
                enterFlag = true;
                break;
            end
        end
...
function [keep_KNN_Dist_ToQuery, keep_KNN_Index_ToQuery] = SingleSeries_ParMaxHeapBased(targetSeqComplete, validSubSeq, subSeqLen, kNN_Uwant, startFiles, endFiles)
...
for ii = startFiles:1:(endFiles-subSeqLen+1)
    ...
    drop_val(:) = cutQuery(1, :);
    if( (target_data_sig(ii, :) ~= 0) && (validSubSeq(ii,1) == 1)  )
...
function [keepAllTargetTogether, keepDataFileInfo, validSubSeq] = ConcatenateAllSeries(subFoldersTarget, subSeqLen )
