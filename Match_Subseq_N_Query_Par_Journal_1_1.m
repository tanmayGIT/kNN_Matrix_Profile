function Match_Subseq_N_Query_Par_Journal_1_1(subFoldersTarget, querySeqComplete, subSeqLen,...
    kNN_Uwant, whichDimToConsider, keepTargetLabels, queryLabel, queryIndex)
...
mergeCompleteAllIndexes = zeros(lenNormal+lenCrossing,numOfpossibleQuery);
...
mergeCompleteAllIndexes(lenNormal+1:end,:) = sorted_CrossingidxActual(1:end,:);
...
for goEachQuery = 1:1:numOfpossibleQuery
    ...
    for iNN = 1:1:kNN_Uwant
        ...
        if (mergeCompleteAllLookUp(getSortedIdx,goEachQuery) > 0) % that means it is a normal distance obtained from normal grouping operation
            ...
            if(isfinite(sorted_Disval(iNN,goEachQuery))) % if the distance at that index is finite
                ...
                if((seriesStart <= pickIndex) && (seriesEnd >= pickIndex) )
                    ...
                    % Now if it happens that the particular sub-sequence is overpassing this particular file limit and it goes upto
                    % the second file then we need to properly include two files
                    if((pickIndex+subSeqLen) > seriesEnd)
                        file_1_start = forward;
                        file_1_end = file_1_start +  (seriesEnd - pickIndex) ;
                        file_2_start = 1;
                        file_2_end = subSeqLen - ((file_1_end - file_1_start)+1);
                        
                        if ( (((file_1_end - file_1_start)+1) + ((file_2_end - file_2_start)+1)) ~= subSeqLen)
                            error('Then there is some problems');
                        end
                        nextFileNam = getAllDataFileInfo.NextFileNum;
                        % fprintf('The Nearest Neighbor_%d is from %s which starts from %d and ends at %d and from %s which starts at %d and ends at %d \n',...
                        %                iNN,fileFullName,file_1_start, file_1_end, nextFileNam, file_2_start, file_2_end);
                        
                        keepAllNearestNNInfo{iNN, goEachQuery}.Indicator = 1; % means there are the existence of second file
                        keepAllNearestNNInfo{iNN, goEachQuery}.NNIndex = iNN;
                        
                        keepAllNearestNNInfo{iNN, goEachQuery}.FirstFileName = fileFullName;
                        keepAllNearestNNInfo{iNN, goEachQuery}.IntialFileStart = file_1_start;
                        keepAllNearestNNInfo{iNN, goEachQuery}.InitialFileEnd = file_1_end;
                        
                        keepAllNearestNNInfo{iNN, goEachQuery}.LastFileName = nextFileNam;
                        keepAllNearestNNInfo{iNN, goEachQuery}.LastFileStart = file_2_start;
                        keepAllNearestNNInfo{iNN, goEachQuery}.LastFileEnd = file_2_end;
                    else
        ...
        elseif(mergeCompleteAllLookUp(getSortedIdx,goEachQuery) == -5)
            % pickIndex = mergeCompleteAllIndexes(getSortedIdx,goEachQuery);
            compInfoCross = mergeCompleteAllInfo{getSortedIdx,goEachQuery};
            
            firstFileStart = compInfoCross.FirstFileStart;
            firstFileEnd = compInfoCross.FirstFileEnd;
            lastFileIndx = compInfoCross.FirstFileName;
            
            secondFileStart = compInfoCross.SecondFileStart;
            secondFileEnd = compInfoCross.SecondFileEnd;
            firstFileIndx = compInfoCross.SecondFileName;
            
            keepAllNearestNNInfo{iNN, goEachQuery}.Indicator = 1; % Means there exist the second file
            keepAllNearestNNInfo{iNN, goEachQuery}.NNIndex = iNN;
            
            keepAllNearestNNInfo{iNN, goEachQuery}.FirstFileName = lastFileIndx;
            keepAllNearestNNInfo{iNN, goEachQuery}.IntialFileStart = firstFileStart;
            keepAllNearestNNInfo{iNN, goEachQuery}.InitialFileEnd = firstFileEnd;
            
            keepAllNearestNNInfo{iNN, goEachQuery}.LastFileName = firstFileIndx;
            keepAllNearestNNInfo{iNN, goEachQuery}.LastFileStart = secondFileStart;
            keepAllNearestNNInfo{iNN, goEachQuery}.LastFileEnd = secondFileEnd;
        end
    end
end
toc;

plotTheGraphOnly2NN(keepFileDistEntryOnly, 1, 2, 1, 2, queryIndex); % comparison between 1NN and 2NN
plotTheGraphOnly2NN(keepFileDistEntryOnly, 1, 4, 1, 5, queryIndex); % comparison between 1NN and 4NN
